...
| [reference/tax-code-bas-visibility.md](reference/tax-code-bas-visibility.md) | A line can post the right amount to the right account and still be invisible to Manager's BAS report if its `TaxCode` is missing — detection/fix pattern across all six taxable-line entity types |
...
9. **Before running a delta migration, ask the human — explicitly, every run — two separate questions, not one: (a) whether every MYOB bank transaction up to the target cutoff has already been categorised/matched in MYOB's own "Bank transactions" screen, and (b) whether every bank transaction already imported into *Manager's own* bank feed has likewise been categorised/matched there, as far as possible.** Don't assume either, and don't infer either from the other — they are different systems' independent backlogs and one being clean says nothing about the other. Confirmed 2026-08-13 for (a): MYOB's own accrual-side journal and its bank feed's cash-clearing record can genuinely disagree on the date for one real transaction (e.g. a superannuation payment journalled 30/6 that the bank statement actually clears 1/7) — ordinary, not a bug, but it surfaces as a false-looking Manager-vs-MYOB reconciliation gap that has nothing to do with Manager at all, and can burn real investigation time before the true cause (MYOB's own two data sources disagreeing) is found. An Unmatched MYOB bank line is a second, related risk: it exists in MYOB's raw feed but never appears in the Journal entries export at all (that export only contains categorized/posted transactions), so a bill/payment reconciliation gate that only reads the journal export will misread a genuinely-present-but-uncategorized MYOB transaction as Manager-only. For (b): an uncategorised Manager bank line sits as an unlinked (Suspense-parked) Payment/Receipt — it exists in Manager's ledger, so it *does* count toward Manager's account total, but with no invoice/contact link it can't be matched against MYOB's own categorized side by anything other than raw amount/date, producing the same kind of false-looking gap from the opposite direction. Both risks shrink to nothing once both systems' bank feeds are fully matched/categorised before the cutoff being migrated.

10. **`config/last_migration_date.txt` must not advance until a reconciliation dashboard shows every account clean, not just until the delta-apply scripts report nothing new to create.** Confirmed as explicit project decision 2026-08-15. `delta_migrate.py`'s own dedup check (`remaining_candidates()`) only covers new Purchase/Sales Invoices and standalone Journal Entries — it says nothing about whether existing bank, GST, AP, AR, or equity accounts actually reconcile. A "clean" delta-apply pass (0 new candidates) can still sit on top of a real, unresolved gap elsewhere entirely — see [reference/live-trial-balance.md](reference/live-trial-balance.md) and this project's own `docs/MIGRATION_DIFFS.md` for real examples found this way. If a project has a reconciliation dashboard script (`scripts/reconciliation_dashboard.py` — project-specific, not part of this skill, since its account list is tailored per business), `delta_migrate.py` runs it automatically as a second gate after the dedup check and refuses to advance the cutoff if it exits nonzero (i.e. any account still shows a real, undocumented diff). Projects without one only get the weaker dedup-only check — building one is worth doing before relying on this workflow for anything beyond a single account or two.
...
- **MYOB Trial balance** (Business Lite): no Cash/Accrual toggle; period is a **month** picker → treat as **month-end** (Jun 2026 ⇒ 30/06/2026). **For a reconciliation check at an arbitrary (non-month-end) date, use MYOB's Balance Sheet report instead** — it does take a custom "as at" date — and negate Liability (2-xxxx) + Equity (3-xxxx) amounts before comparing (MYOB's Balance Sheet shows every account's own *natural* positive balance, unlike the TB export's debit-positive/credit-negative convention; Asset (1-xxxx) amounts need no change). Confirmed exact via a same-date cross-check against a real TB export (0 mismatches across 57 common accounts). Covers Balance Sheet accounts only — no P&L in this report at all — but that's no real loss: the existing TB-based "Current" checkpoint already excludes P&L from comparison too (cumulative-since-inception Manager export vs FY-to-date MYOB, see `reconcile_manager_to_myob.py`'s own docstring). Project implementation: `parse_myob_balance_sheet()` in that same script.
...
- **MYOB's own aggregate Balance Sheet/Trial Balance report exports can be stale relative to MYOB's own journal-level data, even when both were generated within minutes of each other and both are labeled with the same "as at" date.** Confirmed 2026-08-14: a real correcting journal was visible in a freshly-harvested `journal_entries_FY*.xlsx` but absent from a Balance Sheet/TB export generated around the same time — the aggregate report's own generation appears to lag its underlying data by an unpredictable amount, not by a fixed or obvious delay. **Before trusting a `reconcile_manager_to_myob.py` mismatch as real, cross-check the affected account via `audit_account_vs_myob.py` (which reads the journal-level export directly) — if that shows a clean match, the aggregate report is stale, not Manager.** The fix is always the same: re-export the stale MYOB report fresh (Balance Sheet and Trial Balance are separate report screens, both may need re-exporting) and re-run the gate.
- **A `journal_entries_FY*.xlsx` harvest must be bounded strictly to its own fiscal year, not to "today."** `download_journals.py --fy N` defaults `--date-to` to today if not passed explicitly — for the current, still-open fiscal year this silently pulls data past the FY boundary (e.g. `--fy 2026` on 14/8/2026 covers through 14/8/2026, not just to 30/6/2026), which then **overlaps** whatever separate `journal_entries_FY{N+1}.xlsx` file already exists for the next year. `parse_myob_journals()` (in `audit_account_vs_myob.py`/`audit_gst_accounts.py`) reads every `journal_entries_FY*.xlsx` file in the export directory, so any transaction in the overlapping window gets counted twice on the MYOB side — this manufactures large, entirely spurious mismatches that look exactly like real missing corrections. Always pass an explicit `--date-to` pinned to the fiscal year's own end date (e.g. `--date-to 2026-06-30` for `--fy 2026`) when harvesting a *closed* or *closing* year, and re-harvest the adjacent FY file with a non-overlapping `--date-from` for the still-open period separately.
- **Manager's own `trial_balance_current.pdf` can equally go stale relative to Manager's own live ledger** — a PDF exported before a correcting entry was created in Manager (even one created via ordinary API writes, not just manual UI edits) simply won't reflect it, the same staleness risk as MYOB's own exports. Re-export it fresh (`manager-automation`'s `scripts/manager_playwright/export_trial_balance.py`, pinned via `--to-date` to the exact checkpoint date being reconciled) immediately before trusting `reconcile_manager_to_myob.py`'s output, rather than reusing whatever PDF happens to already be sitting in `exports/manager/`.
...
- **MYOB clears pay-run net pay one of two ways, and the difference matters for migration even though both are dollar-correct as originally booked.** Pattern 1 (pre-STP): the "Pay run" transaction posts straight to the real bank account — debit Wages, credit PAYGW Payable, debit/credit Superannuation, net amount credited direct to the bank, no clearing account at all. Pattern 2 (post-STP/batched electronic payment): the same journal instead credits MYOB's **Electronic Clearing Account** (typically an Asset, often `1-3000`, an ABA-file parking lot), and a second, separate transaction — same day or, per the ABA batch's own processing delay, days to weeks later — debits that account and credits the real bank. Until that second step, the ECA holds a **credit-balance asset** (it goes negative). Manager's equivalent — the builtin **Employee clearing account** — uses the same Cr-on-payrun / Dr-on-payment mechanics but is a **Liability**, which is the correct presentation (wages payable). **Detection is which account each historical row actually credits, not a date** — the switchover is specific to when each business adopted electronic/STP payment (confirmed real case, Lilith Pty Ltd: bank-direct until 13/05/2024, Electronic Clearing Account from that date on) and isn't portable across clients. Native Manager payslips require the builtin control, not the migrated chart asset — see `manager-automation`'s [reference/payroll.md](../manager-automation/reference/payroll.md). Migrating the accrual leg as a generic journal against whatever account the Electronic Clearing code was seeded to is wrong regardless of pattern — that account is a chart lookalike, not the builtin control, and a plain journal line on it can never populate per-employee Payslip balances.
  - **Pattern 1 (bank-direct) is not wrong as originally booked, but it is not full native-payroll fidelity either.** A plain multi-line Payment never touches the builtin Employee clearing account, so it produces no Payslip history and no per-employee data. **If the target Manager instance uses native Payslips at all, every historical direct-to-bank pay run needs the same correction as Pattern 2** — it is not "fine as migrated, no action needed" just because the dollar figures were already right.
  - **Correct migration treatment, either pattern:** (1) create a native Manager Payslip for each historical pay run with its exact figures (gross earnings, PAYGW, super) via the existing Payslip Items; (2) repoint the linked bank Payment — the transaction that actually moved cash, whatever account it used (direct bank-expense split, or the Electronic Clearing lookalike) — to the builtin Employee clearing account, same net amount; (3) don't delete an accrual-side journal that's already correct except for the account — a same-day reversal is cleaner than deleting when the API/tooling can't delete records (P&L nets to zero, Balance Sheet was already flat by the cycle's own end).
  - **Known API limitation:** Manager's Payment write endpoint has no employee-tag field on payment lines (schema only allows Account/Amount/Description/LineDescription/AccountsPayable*), even when the line's Account is the builtin Employee clearing control. A Payslip's own net-pay credit is employee-tagged automatically; a retrofitted Payment written via this API is not, and posts to the control account untagged — same "Suspense" failure shape as untagged builtin AR/AP (see `manager-automation`'s live-TB note). Flag this explicitly rather than assuming a scripted fix achieves full per-employee fidelity on the payment side — verify in the Manager UI whether the employee tag can be added manually, and don't treat an API-only result as complete without checking.
  - If native payslips are never going to be used at all, reclassifying the migrated ECA from Asset to Liability (or posting journals to any ordinary wages-payable liability) is enough — the Payslip/Payment retrofit above is only needed for per-employee payslip balances.
- **MYOB's Journal entries export gives every `Pay run` transaction a `description` of the business's own postal address, not a real memo** (e.g. `"40 Elgin Road\nBEECHWORTH  VIC  3747\nAustralia"`) — confirmed 2026-08-14 across three separate pay runs in `journal_entries_FY2026.xlsx`. `Pay run` is also **not actually captured by any automated pipeline in this project** despite being listed in `filter_delta.py`'s `CAPTURED_TXN_TYPES` and described that way in [reference/runbook.md](reference/runbook.md)'s "Recovering deferred non-bank journals" — per `manager-automation`'s [reference/payroll.md](../manager-automation/reference/payroll.md), "past pay runs migrate as ordinary journals," meaning each one is hand-built. Three payroll journals were found live in Manager with this literal address text as their Narration (`journal_dictionary.tsv`'s `description` column, copied verbatim into the `Narration` field) — real, balanced, correctly-coded (Wages & salaries / PAYGW / Superannuation / net pay to the clearing account), just undescribed. **Never copy a `Pay run` row's `description` field into a Manager Narration verbatim** — build one from the pay date and the journal's own Wages/PAYGW/Super lines instead (e.g. `"Payroll pay run <date> -- wages $X, super $Y, PAYGW $Z"`).
- **A migrated line can post the correct amount to the correct account, including a real GST account, and still be silently invisible to Manager's BAS/Tax Summary report** — the report reads `Lines[].TaxCode`, not the resulting GL balance, so a correctly-coded, correctly-balanced transaction with no `TaxCode` on its lines leaves the Trial Balance/GL clean while G1/G11/1A/1B are short. Confirmed across three entity types on a live reconciliation (2026-09-01): an uncoded Purchase Invoice, a manually-pre-split Journal Entry (GST already calculated and posted to `2-2200 GST collected`, just not tax-coded), and an uncoded recurring bank-fee Payment. **This affects all six taxable-line entity types this project writes — Purchase Invoice, Sales Invoice, Credit Note, Debit Note, Journal Entry, Receipt, Payment — not just invoices**; any tax-code audit or fix script must scan all six, not just `apply_bills_invoices.py`'s Purchase/Sales Invoice path. Manager's Journal Entry form has no MYOB-style "Display in GST report as Purchase/Sale" toggle — put the `TaxCode` on the **Credit** line to an **income** account (tax-inclusive amount, `AmountsIncludeTax: true`) to make Manager auto-split it as a sale; a Debit-side tax-coded line reads as a purchase instead. See [reference/tax-code-bas-visibility.md](reference/tax-code-bas-visibility.md) for the full failure mode, the batch-fix heuristic (don't blanket-default to GST Free — check for a sibling line already posting to a GST account, and exclude the tax authority as counterparty, before defaulting uncoded lines to GST Free), and the MYOB `GNR` = Manager `GST Free` mapping gap.
...
- When Manager and MYOB disagree after load: check project `docs/MIGRATION_DIFFS.md` and `config/myob_migration_diffs.tsv` before inventing fixes.
